...
class Model(nn.Module):
    def __init__(self, spec: ModelSpec) -> None:
        ...
        if LAYOUT == "flat":
            ...
            if SCRATCH_MULT:
                # blank, place-indexed working-memory slots.  x*x is a 2P-digit
                # intermediate and the prompt has nowhere to hold it; these give
                # the residual stream a place-aligned scratchpad.  Never read out.
                self.scratch_query = nn.Parameter(torch.zeros(D_MODEL))
                self.scratch_place = nn.Embedding(table * 2, D_MODEL)
        self.block = Block()
    ...
    def _embed_slots(self, input_ids: Tensor, mask: Tensor):
        ...
        if SCRATCH_MULT:
            n_scratch = SCRATCH_MULT * n_places
            s_idx = torch.arange(n_scratch, device=device)
            parts.append(
                (self.scratch_query + self.scratch_place(s_idx))
                .unsqueeze(0)
                .expand(input_ids.shape[0], n_scratch, D_MODEL)
            )
        if ANS_SLOTS:
