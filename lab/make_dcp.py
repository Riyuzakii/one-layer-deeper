#!/usr/bin/env python
"""Emit a self-contained submission: marker-relative parser + weight-tied step.

  --front  marker | rev | abs      prompt -> place-valued digit slots
  --readout phase | digit          the per-iteration squaring step
  --loops N                        weight-tied applications of the step
  --vinit                          structured (still trainable) pointer init

Everything the submission does is learned from random init; the generator writes
no arithmetic, no modulus and no carry rule into the forward pass.
"""
...
TEMPLATE = '''"""digit-carry: marker-relative prompt parser + weight-tied squaring step.
...
VINIT = {vinit}
ANNEAL_STEPS = {anneal}
RAND_LOOPS = {rand_loops}
LR = {lr}
WD = {wd}
DIGIT_TAU = {digit_tau}
VINIT_SCALE = {vinit_scale}
_MAX_STEPS = None
_BATCH_SIZE = None
O_LO, O_HI = -2, 9
...
def main() -> int:
    ...
    ap.add_argument("--front", default="marker", choices=("marker", "rev", "abs"))
    ap.add_argument("--readout", default="phase", choices=("phase", "digit"))
    ...
    ap.add_argument("--rand-loops", type=int, default=1)
    ap.add_argument("--lr", type=float, default=1e-2)
    ap.add_argument("--wd", type=float, default=0.1)
    ap.add_argument("--digit-tau", type=float, default=1.0)
    ap.add_argument("--vinit-scale", type=float, default=4.0)
    args = ap.parse_args()

    src = TEMPLATE.format(
        d_model=args.d_model, n_freq=args.freqs, n_harm=args.harm,
        loops=args.loops, front=args.front, readout=args.readout,
        vinit=bool(args.vinit), anneal=args.anneal,
        rand_loops=args.rand_loops, lr=args.lr, wd=args.wd,
        digit_tau=args.digit_tau, vinit_scale=args.vinit_scale,
    )
    out = Path(args.out)
